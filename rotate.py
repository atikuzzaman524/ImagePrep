...
def rotate_img(image_path, degree, rot_box):
    image_obj = Image.open(image_path)
    rotate_img = image_obj.rotate(-degree, resample=Image.BICUBIC)
    draw = ImageDraw.Draw(rotate_img)
    draw.rectangle(((rot_box[0], rot_box[1]), (rot_box[2], rot_box[3])), outline="black")
    rotate_img.show()
...
def rotate_xml(image_path, deg, x_center, y_center):
        ...
        if child.tag == "object":
            for subchild in child:
                if subchild.tag == "bndbox":
                    ...
                    # rotation:
                    # First lets define the four corners and center of image
